...
import { analyzeCursorSession, toShadowJsonDocument } from "../src/analyze.js";
...
describe("analyzeCursorSession", () => {
  it("never mutates the original transcript file", async () => {
    const path = fixturePath("conformance/operations.cursor.jsonl");
    const before = await readFile(path);
    const beforeHash = sha256(before);
    await analyzeCursorSession({ path });
  ...
  it("never mutates an in-memory event list", async () => {
    ...
    await analyzeCursorSession({ events: frozen });
    expect(JSON.stringify(frozen)).toBe(snapshot);
  });

  it("returns provider-independent shadow statistics including effective tokens", async () => {
    const result = await analyzeCursorSession(fixturePath("conformance/operations.cursor.jsonl"));
    expect(result.source).toBe("cursor");
    expect(result.sessionId).toBe("ops-cursor");
    expect(result.cwd).toBe("/Users/user/project");
    expect(result.cursorVersion).toBe("3.21.16");
  ...
  it("formats the Cursor shadow summary", async () => {
    const result = await analyzeCursorSession(fixturePath("conformance/operations.cursor.jsonl"));
    const text = formatShadowAnalysis(result);
    expect(text).toContain("Context Engine — Cursor Shadow Analysis");
    expect(text).toContain("Mode: SHADOW");
    expect(text).toContain("Cursor version: 3.21.16");
    expect(text).toContain("Session: ops-cursor");
    expect(text).toContain("Effective context");
    expect(text).toContain("Shadow mode only.");
    expect(text).toContain("No Cursor context was modified.");
  });

  it("explains items without dumping huge output", async () => {
    const result = await analyzeCursorSession(fixturePath("cross-agent/auth.cursor.jsonl"), {
      previewLength: 80,
    });
  ...
  it("omits source-content previews when reportPreviews is false", async () => {
    ...
    const document = toShadowJsonDocument(result, "operations.cursor.jsonl");
    expect(document.metadata.source).toBe("cursor");
    expect(document.metadata.localOnly).toBe(true);
    expect(JSON.stringify(document.items)).not.toContain('"preview"');
  });

  it("reuses SemanticProvider without Cursor-specific classification logic", async () => {
  ...
  it("matches a direct compact() of the parsed transcript", async () => {
    ...
    const shadow = await analyzeCursorSession(fixturePath("conformance/operations.cursor.jsonl"));
  ...
  it("writes a shadow report without copying the transcript", async () => {
    ...
    try {
      ...
      expect(saved).toContain('"source": "cursor"');
  ...
  it("does not write into the source file even when asked to save", async () => {
    ...
    const result = await analyzeCursorSession({ path: source });
