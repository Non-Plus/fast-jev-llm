...
import type { CursorTranscriptInput } from "./types.js";
...
function inputToStream(input: CursorTranscriptInput): {
  stream: Readable;
  sourcePath?: string;
} {
  ...
  throw new Error("Cursor JSONL input is not streamable");
}

export async function* readJsonlLines(
  input: Exclude<CursorTranscriptInput, { readonly events: readonly unknown[] }>,
...
export function sourcePathOf(input: CursorTranscriptInput): string | undefined {
  if (typeof input === "string") {
    return input;
  }
