...
describe("Claude privacy", () => {
  it("keeps default analysis local with semantic mode off", async () => {
    ...
    const result = await analyzeClaudeSession(fixturePath("privacy-env.jsonl"), {
  ...
  it("does not send Claude .env secrets to a remote provider", async () => {
    const token = `sk-${"a".repeat(24)}`;
    const provider = new MockSemanticProvider({ defaultResponse: { action: "DROP" } });
    const parsed = await parseClaudeJsonl(fixturePath("privacy-env.jsonl"));
    const result = await compact(parsed.transcript, {
      config: { semanticMode: "remote", recentItemCount: 0 },
      semanticProvider: provider,
    });
    const envItem = result.items.find(
      (item) => item.tool?.path === ".env" || item.tool?.normalizedPath === ".env",
    );
    ...
    expect(
      provider.calls.some((call) =>
        call.candidates.some((candidate) => candidate.itemId === envItem?.id),
      ),
    ).toBe(false);
    expect(parsed.transcript.messages.some((message) => message.content.toString().includes(token))).toBe(
      true,
    );
  });

  it("applies the same redaction gate through analyzeClaudeSession", async () => {
    const token = `sk-${"a".repeat(24)}`;
