...
import type { ClaudeTranscriptInput } from "./types.js";
...
function inputToStream(input: ClaudeTranscriptInput): {
  stream: Readable;
  sourcePath?: string;
} {
  ...
  throw new Error("Claude JSONL input is not streamable");
}

export async function* readJsonlLines(
  input: Exclude<ClaudeTranscriptInput, { readonly events: readonly unknown[] }>,
...
export function sourcePathOf(input: ClaudeTranscriptInput): string | undefined {
  if (typeof input === "string") {
    return input;
  }
